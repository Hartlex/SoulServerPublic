...
namespace DatabaseProxy
{
    static class DatabaseFunctions
    {
        ...
        public static int FindFreeCharacterSlot(int userID)
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand(SelectString("charSlot", "Character", "UserID", userID.ToString())+"AND [DeleteCheck]=0",conn);
                SqlDataReader reader = cmd.ExecuteReader();
        ...
        public static bool getAllCharacters(int userID,out List<PacketStructs.CharacterInfo> characterInfos)
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand(SelectString("*","Character","UserID",userID.ToString())+ "AND [DeleteCheck]=0",conn);
